...
export default function EmotionBoard() {
    ...
    fetch(`${BACKEND}/users/historique`, {
      ...
      .then((data) => {
        ...
        if (data.result) {
          ...
          dispatch(
            saveEmotion({
              emotionName: selected.emotionName,
              emotionImage: selected.imageUrl,
              emotionContent: selected.description,
            })
          );
  ...
  useEffect(() => {
    ...
    if (journal.savedEmotion) {
      ...
      }, 36000000); // 1 heure en millisecondes
    }
  ...
  return (
    <>
      ...
      <View style={styles.container}>
        <View style={styles.emotionBoard}>{emotionSelection}</View>
      </View>
    </>
  );
}
